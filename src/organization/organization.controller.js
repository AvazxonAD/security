...
const organizationGet = async (req, res) => {
  try {
    ...
    const { data, total } = await getorganizationService(
      user_id,
      search,
      offset,
      limit
...
const organizationGetById = async (req, res) => {
  try {
    ...
    const result = await getByIdorganizationService(
      user_id,
      id,
      true,
      req.i18n
...
const organizationUpdate = async (req, res) => {
  try {
    ...
    const data = validationResponse(organizationValidation, req.body);
    const old_data = await getByIdorganizationService(
      user_id,
      id,
      null,
      req.i18n
    ...
    for (let item of data) {
      if (!item.name) {
        throw new ErrorResponse(
          `Row contains empty 'name' field: ${JSON.stringify(item)}`,
          400
        );
      }
