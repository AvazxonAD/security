...
const workerTaskCreate = async (req, res) => {
    try {
        ...
        console.log(task.remaining_task_time)
...
const workerTaskGetById = async (req, res) => {
    try {
        ...
        const result = await getByIdWorkerTaskService(id, true);
        resFunc(res, 200, result);
    } catch (error) {
        errorCatch(error, res);
    }
...
const workerTaskUpdate = async (req, res) => {
    try {
        const user_id = req.user.id
        const id = req.params.id;
        const oldData = await getByIdWorkerTaskService(id)
        ...
        const result = await workerTaskUpdateService(id, worker_id, task_time, task, oldData);
        resFunc(res, 200, result);
    } catch (error) {
        errorCatch(error, res);
    }
