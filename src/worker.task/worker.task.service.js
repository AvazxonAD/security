...
const workerTaskCreateService = async (task, workers, user_id) => {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const promises = [];
    const one_time_summa =
      task.result_summa / task.worker_number / task.task_time;
    for (let worker of workers) {
      const summa = one_time_summa * worker.task_time;
      const query = `INSERT INTO worker_task(worker_id, task_id, summa, task_time, user_id) VALUES($1, $2, $3, $4, $5) RETURNING *`;
  ...
  try {
    ...
    const workers = await pool.query(
      `
            ...
            LEFT JOIN users u ON u.id = w_t.user_id
            WHERE w_t.task_id = $1 
...
const workerTaskUpdateService = async (
  id,
  worker_id,
  task_time,
  task,
  ...
  try {
    ...
    await client.query(
      `UPDATE task SET remaining_task_time = $1 WHERE id = $2`,
      [task.remaining_task_time + oldData.task_time - task_time, task.id]
    );
    await client.query(`COMMIT`);
...
const getByTaskIdANDWorkerIdWorkerTaskService = async (
  task_id,
  worker_id,
  user_id,
  lang
...
const deleteByTaskIDWorkerTaskService = async (task_id) => {
  try {
    await pool.query(
      `UPDATE worker_task SET isdeleted = true WHERE task_id = $1`,
      [task_id]
    );
  } catch (error) {
    throw new ErrorResponse(error, error.statusCode);
