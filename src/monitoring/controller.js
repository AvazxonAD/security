...
const prixodRasxod = async (req, res) => {
  try {
    ...
    const { account_number_id, from, to, page, limit, excel } =
      validationResponse(prixodRasxodQueryValidation, req.query);
    ...
    const { docs, from_summa, to_summa, internal_summa } =
      await MonitoringService.get({
        user_id,
        account_number_id,
        from,
        to,
    ...
    const meta = {
      ...
      from_balance:
        returnStringSumma(Math.round(from_summa.summa * 100) / 100) || 0,
      to_balance:
        returnStringSumma(Math.round(to_summa.summa * 100) / 100) || 0,
      prixod:
        returnStringSumma(Math.round(internal_summa.prixod * 100) / 100) || 0,
      rasxod:
        returnStringSumma(Math.round(internal_summa.rasxod * 100) / 100) || 0,
    };
...
const monitoring = async (req, res) => {
  try {
    const user_id = req.user.id;
    const { year, month, batalon_id } = validationResponse(
      monitoringQueryValidation,
      req.query
    );
    if (batalon_id) {
